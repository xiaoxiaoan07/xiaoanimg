...
fn throw_and_return_null(env: &mut JNIEnv, msg: &str) -> jstring {
...
pub extern "system" fn Java_native_PayloadExtractNative_open(
    ...
    let input_str = match get_string(&mut env, &input_path) {
        Ok(s) => s,
        Err(e) => {
    ...
    match input::open(&input_str, false) {
        ...
        Err(e) => {
            let _ = env.throw_new(
                ...
                &format!("Failed to open payload: {e}"),
            );
            0
        }
    }
...
pub extern "system" fn Java_native_PayloadExtractNative_listPartitionsJson(
    ...
    let json = match serde_json::to_string(&entries) {
        Ok(s) => s,
        Err(e) => return throw_and_return_null(&mut env, &format!("JSON serialization failed: {e}")),
    };

    match env.new_string(&json) {
        Ok(s) => s.into_raw(),
        Err(e) => throw_and_return_null(&mut env, &format!("Failed to create Java string: {e}")),
    }
}
...
pub extern "system" fn Java_native_PayloadExtractNative_extractPartition(
    ...
    let input_str = match get_string(&mut env, &input) {
        Ok(s) => s,
        Err(e) => {
            let _ = env.throw_new("java/lang/RuntimeException", &e);
            return;
    ...
    let output_str = match get_string(&mut env, &output_dir) {
        Ok(s) => s,
        Err(e) => {
            let _ = env.throw_new("java/lang/RuntimeException", &e);
            return;
    ...
    let part_name = match get_string(&mut env, &partition_name) {
        Ok(s) => s,
        Err(e) => {
            let _ = env.throw_new("java/lang/RuntimeException", &e);
            return;
    ...
    let payload = match input::open_for_extract(&input_str, &partition_names, false) {
        ...
        Err(e) => {
            let _ = env.throw_new(
                "java/lang/RuntimeException",
                &format!("Failed to open payload for extraction: {e}"),
            );
            return;
        }
    };
    ...
    if let Err(e) = extract::extract_partitions(&payload, output_path, &partition_names, &config) {
        let _ = env.throw_new(
            "java/lang/RuntimeException",
